...
import {
  mergeConfig,
  resolveConfig,
} from "./node_modules/astro/dist/core/config/index.js";
...
export async function loadFixture(inlineConfig) {
  ...
  // Handle "./fixtures/...", convert to absolute path
    ...
    throw new Error(
      "Must provide { root: '.../fixtures/...' } with an absolute path",
    );
  }
  ...
  const resolveUrl = (url) =>
    `http://${config.server.host || "localhost"}:${
      config.server.port
    }${url.replace(/^\/?/, "/")}`;
  ...
  return {
    ...
    pathExists: (p) =>
      fs.existsSync(new URL(p.replace(/^\//, ""), config.outDir)),
    ...
    readdir: (fp) =>
      fs.promises.readdir(new URL(fp.replace(/^\//, ""), config.outDir)),
    glob: (p) =>
      fastGlob(p, {
    ...
    loadEntryPoint: async (pagePath, routes, streaming) => {
      const virtualModule = getVirtualModulePageNameFromPath(
        RESOLVED_SPLIT_MODULE_ID,
        pagePath,
      );
      const filePath = makeSplitEntryPointFileName(virtualModule, routes);
      const url = new URL(
        `./server/${filePath}?id=${fixtureId}`,
        config.outDir,
      );
      const { createApp, manifest } = await import(url);
      const app = createApp(streaming);
...
export async function cliServerLogSetup(flags = [], cmd = "dev") {
  ...
  const local = messages
    .find((msg) => msg.includes("Local"))
    ?.replace(/Local\s*/g, "");
  const network = messages
    .find((msg) => msg.includes("Network"))
    ?.replace(/Network\s*/g, "");
