...
export async function createRoutesFile(
	...
	const OPTION1_TOTAL_COUNT = 1 + EXTENDED_INCLUDE_RULES_COUNT + EXCLUDE_RULES_COUNT;
	const OPTION2_TOTAL_COUNT =
		INCLUDE_RULES_COUNT + (includedPathsHaveWildcard ? EXCLUDE_RULES_COUNT : 0);

	if (
		!hasPrerendered404 ||
		OPTION1_TOTAL_COUNT <= CLOUDFLARE_COMBINED_LIMIT ||
		OPTION2_TOTAL_COUNT > CLOUDFLARE_COMBINED_LIMIT
	) {
